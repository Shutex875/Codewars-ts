...
export function score(dice: number[]): number {
  ...
  for (const [valueStr, count] of Object.entries(counts)) {
    ...
    if (count >= 3) {
      totalScore += value === 1 ? 1000 : value * 100;
      const remaining = count - 3;
      if (value === 1) totalScore += remaining * 100;
      if (value === 5) totalScore += remaining * 50;
    } else {
      if (value === 1) totalScore += count * 100;
      if (value === 5) totalScore += count * 50;
    }
  }
  return totalScore;
}
