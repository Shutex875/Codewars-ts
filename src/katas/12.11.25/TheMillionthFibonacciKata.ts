 ...
 * Gather all of the learned men in Pisa, especially Leonardo Fibonacci. In order for the
 * crusades in the holy lands to be successful, these men must calculate the millionth number
 * in Fibonacci's recurrence. Fail to do this, and your armies
 ...
 * numerals at this moment in history, he cannot represent this number. If he only knew about
 * the invention of zero, it might make this sort of thing easier.
...
function fib(n: number): bigint {
  ...
  function fastFib(k: number): [bigint, bigint] {
    if (k === 0) return [0n, 1n];
    const [fk, fk1] = fastFib(Math.floor(k / 2));
    const f2k = fk * (2n * fk1 - fk);
    const f2k1 = fk * fk + fk1 * fk1;
    if (k % 2 === 0) {
      return [f2k, f2k1];
    } else {
