 ...
 * never finally lock the system or sound the alarm. That's why we can try out all possible (*) variations. * possible
 * in sense of: the observed PIN itself and all variations considering the adjacent digits
 ...
 * the observed one and also the results, must be strings, because of potentially leading '0's. We already
...
function getPINs(observed: string): string[] {
  ...
  function generateVariations(digits: string[], index: number): string[] {
    if (index === digits.length) {
      return [""];
    }
    const currentDigit = digits[index];
    const variations: string[] = [];
    const restVariations = generateVariations(digits, index + 1);
    for (const neighbor of neighbors[currentDigit]) {
      for (const rest of restVariations) {
        variations.push(neighbor + rest);
      }
    }
    return variations;
  }
  return generateVariations(observed.split(""), 0);
}
